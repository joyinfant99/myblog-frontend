...
const BlogPost = ({ editMode: initialEditMode = false }) => {
    ...
    const fetchPost = useCallback(async () => {
        try {
            ...
            // Handle images - support both Cloudinary URLs and legacy local paths
            if (data.bannerImage) {
                const bannerUrl = data.bannerImage.startsWith('http') 
                    ? data.bannerImage 
                    : `${REACT_APP_API_URL}/${data.bannerImage}`;
                setExistingBannerImage(bannerUrl);
                setBannerImagePreview(bannerUrl);
            }
            
            if (data.socialImage) {
                const socialUrl = data.socialImage.startsWith('http') 
                    ? data.socialImage 
                    : `${REACT_APP_API_URL}/${data.socialImage}`;
                setExistingSocialImage(socialUrl);
    ...
    const imageUrl = post.bannerImage 
        ? (post.bannerImage.startsWith('http') ? post.bannerImage : `${REACT_APP_API_URL}/${post.bannerImage}`)
        : null;
                    ...
                    <>
                        ...
                        {post.bannerImage && (
                            <div className="banner-container">
                                <img 
                                    src={post.bannerImage.startsWith('http') ? post.bannerImage : `${REACT_APP_API_URL}/${post.bannerImage}`} 
                                    alt={post.title}
