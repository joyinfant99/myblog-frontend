...
const EditPost = () => {
    ...
    const fetchPost = useCallback(async () => {
        try {
            ...
            if (post.bannerImage) {
                const bannerUrl = post.bannerImage.startsWith('http') 
                    ? post.bannerImage 
                    : `${REACT_APP_API_URL}/${post.bannerImage}`;
                setExistingBannerImage(bannerUrl);
                setBannerImagePreview(bannerUrl);
            }
            
            if (post.socialImage) {
                const socialUrl = post.socialImage.startsWith('http') 
                    ? post.socialImage 
                    : `${REACT_APP_API_URL}/${post.socialImage}`;
